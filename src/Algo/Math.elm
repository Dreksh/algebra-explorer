module Algo.Math exposing (
    ...
    FunctionProperty, functionPropertyDecoder, createConstant, encodeFunctionProperty,
    parse, validVariable,
    encode, decoder
    )
...
type alias FunctionProperty props = Tree props

functionPropertyDecoder: Decode.Decoder props -> Decode.Decoder (FunctionProperty props)
functionPropertyDecoder propDec = propDec
    |> Decode.andThen (\state -> Decode.field "arguments" Decode.int
        |> Decode.andThen (\arguments -> if arguments <= 0
            then Decode.fail "Functions must contain a positive number of integers"
            else if arguments == 1 then Decode.succeed (UnaryNode {state = state, name = "", child = RealNode {state = state, value = 0}})
...
encodeFunctionProperty: (props -> List (String, Encode.Value)) -> FunctionProperty props -> Encode.Value
encodeFunctionProperty stateEnc fp =
    (   case fp of
...
parse: Dict.Dict String {a | property: FunctionProperty prop} -> String -> Result String (Tree (Maybe prop))
...
equation_: Dict.Dict String {a | property: FunctionProperty prop} -> Parser_ (Tree (Maybe prop))
equation_ funcProps = Parser.loop []
    (\list -> if List.isEmpty list
        then Parser.succeed (List.singleton >> Parser.Loop) |. Parser.spaces |= expression_ funcProps |. Parser.spaces
        else Parser.oneOf
    ...
    |> Parser.map (\children -> case children of
        ...
        _ -> let p = Dict.get "=" funcProps |> Maybe.map (.property >> getState) in
            DeclarativeNode {state = p, name = "=", children = children}
    )

expression_: Dict.Dict String {a | property: FunctionProperty prop} -> Parser_ (Tree (Maybe prop))
expression_ funcProps = Parser.loop []
    (\list -> if List.isEmpty list
        then Parser.succeed (List.singleton >> Parser.Loop) |= multiple_ funcProps |. Parser.spaces
        else Parser.oneOf
            [   Parser.succeed (\elem -> Parser.Loop (elem :: list))
                |. expectSymbol_ "+"
                |. Parser.spaces
...
multiple_ funcProps = Parser.loop []
    ...
    |> generateMultipleNode_ funcProps

generateMultipleNode_: Dict.Dict String {a | property: FunctionProperty prop} -> Parser_ (List (Tree (Maybe prop))) -> Parser_ (Tree (Maybe prop))
...
product_: Dict.Dict String {a | property: FunctionProperty prop} -> Parser_ (Tree (Maybe prop))
        ...
        |> Parser.andThen (\(name, props) -> case (Dict.get name funcProps, props) of
            ...
            (Just p, Just children) -> case p.property of
                UnaryNode n -> case children of
                    [child] -> Parser.succeed (UnaryNode {state = Just n.state, name = name, child = child})
...
varOrFunc_: Dict.Dict String {a | property: FunctionProperty prop} -> Parser_ (Maybe (List (Tree (Maybe prop))))
varOrFunc_ funcProps = Parser.oneOf
    [   Parser.succeed Just |. expectSymbol_ "(" |= argsList_ funcProps |. expectSymbol_ ")"
    ,   Parser.succeed Nothing
    ]

argsList_: Dict.Dict String {a | property: FunctionProperty prop} -> Parser_ (List (Tree (Maybe prop)))
...
tokenNumber_: Parser_ (Tree (Maybe p))
tokenNumber_ = Parser.succeed (\a b -> a ++ b) |= tokenDigit_ |= Parser.oneOf [Parser.succeed (\b -> "." ++ b) |. expectSymbol_ "." |= tokenDigit_ , Parser.succeed ""]
    |> Parser.andThen (\str -> case String.toFloat str of
        ...
        Just f -> Parser.succeed (RealNode {state = Nothing, value = f})
    )
