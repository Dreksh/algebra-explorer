...
addTopic topic model = Ok model -- TODO: Actually add it

update: Event -> Model -> (Model, Cmd Event)
update _ model = (model, Cmd.none)

type alias MenuItem_ msg = Bool -> List (Html.Attribute msg) -> String -> List (Html.Html msg) -> List (Html.Html msg)
menuRules: (Event -> msg) -> MenuItem_ msg -> Model -> List (Html msg)
menuRules converter menuItem model = menuItem True [] "Rules"
    (   List.foldl (\topic result -> result ++
            menuItem True [] topic.name
            (   List.foldl (\rule (index, res) -> (index + 1, res ++
                    menuItem
                    True -- TODO: Only show if it matches the selected node
...
menuFunctions converter menuItem model = menuItem True [] "Functions"
    (   Dict.foldl (\name (props, _) result -> result ++
            ...
            ,   p [] [text ("Parameters: " ++ String.fromInt props.parameters)]
...
topicDecoder = Dec.map3 (\a b c -> (a,b,c))
    ...
    (Dec.field "variables" (Dec.list Dec.string))
...
ruleDecoder_ functions variables = Dec.map6 Rule_
    ...
    (Dec.field "parameters"
        (   Dec.keyValuePairs Dec.string
        |>  Dec.andThen (\list ->
            List.foldl (\(key, description) res -> case res of
                Err _ -> res
                Ok others -> case parseExpression_ functions key of
                    Err str -> Err str
                    Ok exp -> Ok (others ++ [(exp, description)])
            ) 
            (Ok [])
            list
            |> resultToParser_
...
matchFunction_ props node =
    if props.associative && props.commutative then
        ...
        props.args == List.length node.args && props.parameters == List.length node.parameters
