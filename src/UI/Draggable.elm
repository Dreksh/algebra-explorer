    ...
    Drag pid to -> case Dict.get (Encode.encode 0 pid) model.drags of
        ...
        Just start ->
            ...
            in case start.direction of
                ...
                Just TopRight -> ({ model | coordinates = {orig | right = orig.right + dx |> max (orig.left + 2), bottom = orig.bottom + dy |> max (orig.top + 2) }}, Nothing)
                Just BottomLeft -> ({ model | coordinates = {orig | left = orig.left + dx |> min (orig.right - 2), top = orig.top + dy |> min (orig.bottom - 2)} }, Nothing)
